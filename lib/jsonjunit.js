        ...
        if (jsonRaw && jsonRaw.toString().trim() !== '') {
            ...
            if (jsonType == 'mochawesome') {
                ...
                var dateFormatted = new Date(jsonData.stats.start),
                  ...
                  if(jsonData.suites.suites[0].suites.length > 0) {
                    suites = jsonData.suites.suites[0].suites;
                    suiteName = jsonData.suites.suites[0].title;
                  } else {
                    suites = jsonData.suites.suites;
                    suiteName = jsonData.suites.suites.title || 'Test Suite';
                  }
                
                writeString('<testsuites name="' + suiteName + '">\n');
